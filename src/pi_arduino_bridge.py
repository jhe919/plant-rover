...
def command():
    ...
    extra = ""
    if "bbox_center_x" in payload or "bbox_width" in payload:
        extra = f" center={payload.get('bbox_center_x')} width={payload.get('bbox_width')}"
    print(f"[bridge] cmd={cmd}{extra}", flush=True)
    ser.write((cmd.strip() + "\n").encode("utf-8"))
